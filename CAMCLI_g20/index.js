...
const createChart = (divId) => {
  ...
  xAxis.get("dateFormats")["year"] = "MM/yyyy";
  ...
  root.dateFormatter.set("dateFormat", "[bold]MM/yyyy");
  ...
  am5.net
    ...
    .then((data) => {
      ...
      parsedData.forEach((el) => {
        if(el.indicador === "anomalia_temperatura_deg_c")
        dataAnomalia.push(el)
      });

      console.log('data', dataAnomalia);
      createLineSeries(dataAnomalia);
    })
    .catch((result) => {
  ...
  series.bullets.push(() => {
    ...
    circle.adapters.add("tooltipHTML", function (text, target) {
      if (target.dataItem) {
        ...
        let divTool = `<div class="tooltip"><p class="titu">Anomalía de temperatura</p><p class='data'><span class='punto'>&#9679</span><span class='anio'>${new Date(dataItem.fecha).getMonth()+1}/${new Date(dataItem.fecha).getFullYear()}:</span> ${dataItem.valor.toFixed(2)} ºC</p></div>`;
