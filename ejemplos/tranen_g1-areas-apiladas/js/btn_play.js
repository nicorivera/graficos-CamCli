function btn_play(root, data) {
  ...
  function animateYears(index, onComplete) {
    if (index < years.length) {
      ...
      let currentYear = years[index];

      var endLabel = scrollbarX.endGrip.children._values[0];
      if (currentYear) {
        endLabel.set("text", currentYear.toString());
        xAxis.zoomToCategories(startYear.toString(), currentYear.toString());
      }
  ...
  btn_play.events.on("click", function () {
    ...
    xAxis.zoomToCategories(years[0], years[1]);
    setTimeout(function () {
