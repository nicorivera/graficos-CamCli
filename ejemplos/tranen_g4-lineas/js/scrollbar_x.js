function scrollbar_x(root) {
  ...
  scrollbarX.events.on("rangechanged", function (event) {
    let start = xAxis.positionToDate(event.target.get("start"));
    let end = xAxis.positionToDate(event.target.get("end"));

    startLabel.set("text", start.getFullYear());
    endLabel.set("text", end.getFullYear());
  });
