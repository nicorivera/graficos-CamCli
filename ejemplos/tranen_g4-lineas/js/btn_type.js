function btn_type(chart, root, data) {
  var htmlModalSeries = `
...
`;
  ...
  function addCheckboxes() {
    ...
    var seriesActives = [];
    chart.series.each(function (series, index) {
      seriesActives.push(series.get("name"));
    });
    console.log(seriesActives);

    // chart.series.each(function (series, index) {
    categories.forEach(function (series, index) {
      var seriesId = `series-${index}`;
      ...
      checkbox.addEventListener("change", function () {
        ...
        const selectedLabels = Array.from(selectedCheckboxes).map(
          (checkbox) => {
