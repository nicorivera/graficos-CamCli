function btn_type(root) {
  ...
  var htmlModalSeries = `
    <div class="modal-type relative  w-full md:max-w-xl  md:w-[1000px]">   
      <div class="">
        <div class="">
          <div class="flex md:flex-row flex-col items-center gap-2 justify-between">
            ...
            <a id="close-modal-type" class="absolute md:static -right-8 -top-10 hover:bg-opacity-90 cursor-pointer active:scale-95 origin-center w-7  h-7  p-1.5 flex items-center justify-center  rounded-full  bg-gray-800 text-sm md:text-base ">
          ...
          <div id="divCheckboxes">
            <div id="content-modal-type" class="mt-4 md:mt-8 grid grid-cols-1 md:grid-cols-3 gap-1 md:gap-3"></div>
          </div>
        </div>
      </div>
    </div>
  ...
  function addCheckboxes() {
    ...
    chart.series.each(function (series, index) {
      ...
      var seriesPais = chart.series._values[index]._data._values[0].pais;
      var checkbox = document.createElement("input");
      checkbox.type = "checkbox";
      checkbox.id = seriesId;
      checkbox.value = seriesPais;
      ...
      label.appendChild(document.createTextNode(seriesPais));
