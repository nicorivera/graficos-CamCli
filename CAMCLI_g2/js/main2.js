document.addEventListener("DOMContentLoaded", function () {
  ...
  load_data()
    .then((rows) => {
      ...
      //   let dato = { category: pais.category }
      //   arrayData.push(dato)
      //   // let anio = { anio: pais.anio }
      //   // arrayAnios.push(pais.year)
      //   // if(pais.year == 1750){
      //   //   arrayPaises.push(pais.value)
      //   // }
      // })
      // let jsonObject = arrayData.map(JSON.stringify);
      // console.log('jsonObject', jsonObject);
      // let uniqueSet = new Set(jsonObject);
      // console.log('uniqueSet', uniqueSet);
      // let uniqueArray = Array.from(uniqueSet).map(JSON.parse);
      // console.log('uniqueArray', uniqueArray);
      // let uniqueSetAnios = new Set(arrayAnios);
      // console.log('uniqueSetAnios', uniqueSetAnios);
      // let uniqueArrayAnios = Array.from(uniqueSetAnios);
      // console.log('uniqueArrayAnios', uniqueArrayAnios.sort());
      // console.log('arrayPaises', arrayPaises.sort());
      init();
    })
    .catch((error) => {
  ...
  function init() {
    ...
    tooltip.label.adapters.add("text", function (text, target) {
      ...
      chart.series.each(function (series) {
        ...
        var seriesValue = series.get("valueYField");
        // var seriesName = tooltipDataItem.dataContext.category;
        // var seriesValue = tooltipDataItem.dataContext.value;
        console.log('TOOL', tooltipDataItem);
        ...
        if (tooltipDataItem) {
          ...
          "]●[/] [bold width:100px]" +
            seriesName +
            ":[/] " +
            parseFloat(seriesValue).toFixed(2) +
            "";
        }
        i++;
      });
