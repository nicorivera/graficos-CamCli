...
const createChart = (divId) => {
  ...
  processor = am5.DataProcessor.new(root, {
    numericFields: ["anomalia_temperatura_deg_c"],
    dateFormat: "yyyy",
    dateFields: ["fecha"],
  });
...
const createLineSeries = (parsedData) => {
  ...
  series = chart.series.push(
    am5xy.SmoothedXLineSeries.new(root, {
      ...
      xAxis: xAxis,
      yAxis: yAxis,
      valueYField: "anomalia_temperatura_deg_c",
      valueXField: "fecha",
      locationX: 0.5,
      tension: 0.2,
  ...
  series.bullets.push(() => {
    ...
    circle.adapters.add("tooltipHTML", function (text, target) {
      if (target.dataItem) {
        ...
        let divTool = `<div class="tooltip"><p class="titu">Temperatura</p><p class='data'><span class='punto'>&#9679</span><span class='anio'>${new Date(dataItem.fecha).getFullYear()}:</span> ${dataItem.anomalia_temperatura_deg_c.toFixed(2)} ºC</p></div>`;
