  ...
  am5.net
    ...
    .then((data) => {
      ...
      parsedData.forEach((el) => {
        if(el.indicador === "tendencia_ch4_ppb")
        dataTendencia.push(el)
      });

      // console.log('data', dataTendencia);
      createLineSeries(dataTendencia);
    })
    .catch((result) => {
  ...
  series.bullets.push(() => {
    ...
    circle.adapters.add("tooltipHTML", function (text, target) {
      if (target.dataItem) {
        ...
        let divTool = `<div class="tooltip"><p class="titu">Emisiones de CH4</p><p class='data'><span class='punto'>&#9679</span><span class='anio'>${new Date(dataItem.fecha).getMonth()+1}/${new Date(dataItem.fecha).getFullYear()}:</span> ${dataItem.valor.toFixed(2)} ppb</p></div>`;
