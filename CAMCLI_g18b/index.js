  ...
  am5.net
    ...
    .then((data) => {
      ...
      parsedData.forEach((el) => {
        if(el.indicador === "promedio_ch4_ppb")
        dataPromedio.push(el)
      });

      // console.log('data', dataPromedio);
      createLineSeries(dataPromedio);
    })
    .catch((result) => {
  ...
  series.bullets.push(() => {
    ...
    circle.adapters.add("tooltipHTML", function (text, target) {
      if (target.dataItem) {
        ...
        let divTool = `<div class="tooltip"><p class="titu">Emisiones de N2O</p><p class='data'><span class='punto'>&#9679</span><span class='anio'>${new Date(dataItem.fecha).getMonth()+1}/${new Date(dataItem.fecha).getFullYear()}:</span> ${dataItem.valor.toFixed(2)} ppb</p></div>`;
